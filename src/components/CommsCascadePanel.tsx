...
function DraftRow({ exerciseId, draft }: { exerciseId: string; draft: Draft }) {
  ...
  return (
    <li className={`rounded-md border p-2 text-sm ${STATUS_CLASS[draft.status] ?? "border-slate-200 bg-white"}`}>
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 text-[11px] text-slate-500">
            <span className="rounded-full bg-slate-100 px-1.5 py-0.5">{draft.audience}</span>
      ...
      {rejectOpen && (
        <form
          ...
          className="mt-2 flex gap-2 rounded bg-slate-50 p-2"
        >
          <input type="hidden" name="exerciseId" value={exerciseId} />
          <input type="hidden" name="draftId" value={draft.id} />
          <input
